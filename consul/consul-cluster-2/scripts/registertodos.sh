...
consul config write -http-addr="$OUR_SERVER_ADDRESS:8500" /opt/consul/configs/config.todos.hcl

consul connect envoy -http-addr="$OUR_SERVER_ADDRESS:8500" -grpc-addr="$OUR_SERVER_ADDRESS:8502" -sidecar-for todos-1